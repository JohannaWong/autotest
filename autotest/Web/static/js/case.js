$(function(){
	//改变产品线id,项目ID改变
	jQuery("#projectline_id").change(refresh_projects);
...
function refresh_projects(){
	jQuery.ajax({ 
		type:"GET", 
		url:"ajaxgeteffprojects", 
		data:"productline_id="+jQuery("#projectline_id").val(), 
		//beforeSend:select_script_loading,
...
//响应产品线的变化，刷新project list
function refresh_projects_list(data)
{
	result = eval('(' + data + ')');
	jQuery("#project_id").html("");
	var size = result.length;
	for(i=0;i< size;i++)
	{
		jQuery("#project_id").append("<option value="+result[i].id+">"+result[i].project_name+"</option>")
	}
}
